...
-- После проверки ДЗ: Лучше тут ставить LEFT JOIN, а не просто JOIN, так как в этом случае из левых таблиц подтянется вообще все, например, города без id региона.
-- Когда город без id региона это пример не целостности данных. Это бывает в реальном мире.
SELECT 
    ci.id,
    important,
...
-- После проверки ДЗ: В условии лучше писать вот так r.title LIKE 'Московская%' . Это в реальных данных поможет собрать больше нужных данных.
...
-- После проверки ДЗ: Можно еще использовать функцию CURDATE() в условии для получения текущей даты и, с ее помощью, выбирать только текущие периоды следующим образом:
